describe SignUpSheetController do
      ...
      context 'when an assignment_participant can be found' do
        ...
        context 'when creating team related objects unsuccessfully' do
          it 'shows a flash error message and redirects to assignment#edit page' do
            ...
            allow(User).to receive(:find).with(8).and_return(student)
            allow(Assignment).to receive(:find).with(1).and_return(assignment)
            allow(TeamsParticipant).to receive(:create).with(user_id: 8, team_id: 1).and_return(double('TeamsUser', id: 1))
  ...
  describe '#delete_signup_as_instructor' do
    before(:each) do
      allow(Team).to receive(:find).with('1').and_return(team)
      allow(TeamsParticipant).to receive(:find_by).with(team_id: 1).and_return(double('TeamsUser', user: student))
  ...
  describe '#show_team' do
    it 'renders show_team page' do
      ...
      allow(TeamsParticipant).to receive(:where).with(team_id: 1).and_return([double('TeamsUser', user_id: 1)])
  ...
  describe '#switch_original_topic_to_approved_suggested_topic' do
    it 'redirects to sign_up_sheet#list page' do
      allow(TeamsParticipant).to receive(:where).with(user_id: 6).and_return([double('TeamsUser', team_id: 1)])
      allow(TeamsParticipant).to receive(:where).with(team_id: 1).and_return([double('TeamsUser', team_id: 1, user_id: 8)])
      allow(Team).to receive(:find).with(1).and_return(team)
      team.parent_id = 1
